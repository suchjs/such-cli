...
const runAll = async (isOnlyInstall) => {
  ...
  try {
    ...
    const serverDirName = "serverDir";
    const serverDirDef = "server";
          ...
          ? [
              {
                ...
                name: serverDirName,
                message:
                  "Please enter the mock server's data template file directory:(used for running a mock server, base on such directory)",
                validate: dirnameValidator,
                default() {
                  return serverDirDef;
                },
    ...
    const serverDir = results[serverDirName];
    const confConfig = {
      suchDir: confSuchDir,
    ...
    try {
      ...
      if (isForCli) {
        await mkdir(path.join(lastSuchDir, serverDir));
      }
      await createFile(
        configFile,
