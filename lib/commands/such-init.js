...
const runAll = async (isOnlyInstall) => {
  const confFileExt = packages.type && packages.type === 'module' ? 'cjs' : 'js';
  ...
  const successInfo = `${okSymbol} Everything is ok now,enjoy it!`;
  try {
    await isFileOk(configFile);
    log(isOnlyInstall ? successInfo : `${okSymbol} your config file is exists,just config it.`);
  } catch (e) {
