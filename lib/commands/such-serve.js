...
const { capitalize, hasOwn } = require("../helpers/utils");
...
const server = http.createServer(async (req, res) => {
  ...
  if (matched) {
    ...
    if (exist) {
      ...
      if (ext === ".json") {
        content = await globalSuch.instance(JSON.parse(content)).a(suchOptions);
  ...
  printDebugInfo("The request is not match the prefix or any mock template file", 404);
  res.writeHead(404);
  res.end();
});
