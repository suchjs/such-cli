...
(async () => {
  ...
  const startHttpServer = async () => {
    ...
    const { server: config = {}, extensions = [".json"] } =
      cliSuch.store.config;
    // get the last config
    const lastConfig = {
      port: 8181,
    ...
    httpServer = http.createServer(async (req, res) => {
      ...
      if (prefix) {
        ...
        if (!matched)
          reason = `The url's pathname not match a prefix '${prefix}'${
            hasPrefixExclude ? " or in the exclude pathnames." : ""
          }`;
        if (debug)
          printDebugInfo(
    ...
    const curWatch =
      cliConfig.watch ||
      (cliSuch.store.config.server && cliSuch.store.config.server.watch);
    if (curWatch !== watch) {
